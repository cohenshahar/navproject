...
for run in range(NUM_SIMULATION_RUNS):
    ...
    for sim in range(NUM_SIMULATIONS):
        ...
        for step in range(MAX_STEPS):
            ...
            rewards_this_run.append(reward)
    ...
    success_per_run.append(success_count)
    avg_reward = np.mean(rewards_this_run) if rewards_this_run else 0.0
    avg_reward_per_run.append(avg_reward)

# === Plot Results ===
runs = list(range(1, NUM_SIMULATION_RUNS + 1))
success_rate_per_run = [s / NUM_SIMULATIONS for s in success_per_run]

plt.figure(figsize=(10, 5))
plt.plot(runs, success_per_run, marker='o')
plt.xlabel("Run #")
plt.ylabel("Total Successes")
plt.title("Successes Per Run")
plt.grid(True)
plt.tight_layout()
plt.savefig("successes_per_run.png")
plt.show()

plt.figure(figsize=(10, 5))
plt.plot(runs, success_rate_per_run, marker='o', color='green')
plt.xlabel("Run #")
plt.ylabel("Success Rate")
plt.title("Success Rate Per Run")
plt.grid(True)
plt.tight_layout()
plt.savefig("success_rate_per_run.png")
plt.show()

plt.figure(figsize=(10, 5))
plt.plot(runs, avg_reward_per_run, marker='o', color='purple')
plt.xlabel("Run #")
plt.ylabel("Average Reward")
plt.title("Average Reward Per Run")
plt.grid(True)
plt.tight_layout()
plt.savefig("avg_reward_per_run.png")
plt.show()
